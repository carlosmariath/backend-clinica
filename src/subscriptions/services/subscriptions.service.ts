...
export class SubscriptionsService {
  ...
  async create(createDto: CreateSubscriptionDto): Promise<SubscriptionDto> {
    ...
    if (!therapyPlan) {
      throw new NotFoundException(
        `Plano de terapia com ID ${createDto.planId} não encontrado`,
      );
    }
  ...
  async getConsumptionHistory(
    ...
    if (!existingSubscription) {
      throw new NotFoundException(
        `Assinatura com ID ${subscriptionId} não encontrada`,
      );
    }

    // Buscar o histórico de consumo
    const consumptionHistory =
      await this.prisma.subscriptionConsumption.findMany({
